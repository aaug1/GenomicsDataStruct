...
class CuckooFilter:
  # n = len(items), fp = false positive rate
    def __init__(self, n: int, entries: int, fp: float):
        self.b = entries  # entries per bucket
...
# need to give it how many items you plan to insert, the number of entries (how many columns), and the desired false positive rate
# It will try to optimize the size for the table even if you give a small number of columns by increasing rows
x = CuckooFilter(10, 2, .9)
